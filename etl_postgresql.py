#!/usr/bin/env python3
"""
Urban Mobility Data ETL script - PostgreSQL version
Loads data into vendors, zones, and trips tables for PostgreSQL
"""
...
def get_or_create_vendor(engine, vendor_code):
    ...
    with engine.connect() as conn:
        ...
        result = conn.execute(text("SELECT vendor_id FROM vendors WHERE vendor_code = :vendor_code"), 
        ...
        if row:
            ...
            """), {
                "vendor_code": vendor_code,
                "vendor_name": f"Vendor {vendor_code}"
            })
    ...
    for _, row in df.iterrows():
        ...
        if len(reasons) == 0:
            ...
            if vendor_code and vendor_code not in vendor_cache:
                vendor_cache[vendor_code] = get_or_create_vendor(engine, vendor_code)
...
def insert_trips_postgresql(engine, table_name, rows, batch_size=1000):
    ...
    with engine.connect() as conn:
        ...
        for i in range(0, len(rows), batch_size):
            ...
            values_list = []
            for r in batch:
                values_list.append(tuple(r.get(c) for c in cols))
            ...
            for values in values_list:
                conn.execute(text(insert_sql), dict(zip(cols, values)))
...
def parse_args():
    p = argparse.ArgumentParser(description="ETL (clean + load) into PostgreSQL for NYC Taxi dataset")
    p.add_argument("--input", required=True, help="Path to raw CSV (train.csv)")
    p.add_argument("--database-url", required=True, help="PostgreSQL database URL")
...
def main():
    ...
    try:
        ...
        for chunk in pd.read_csv(args.input, chunksize=args.chunksize, low_memory=False):
            ...
            clean_rows, excluded_rows = clean_chunk(chunk, engine)
            inserted = insert_trips_postgresql(engine, args.table, clean_rows, batch_size=args.batch_size)
